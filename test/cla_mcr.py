...
def main():
    ...
    # In[72]:
    exp_dist = dimension_of_mcr_vectors*(r[1]-r[0]+1)/4
    exp_dist_std = (dimension_of_mcr_vectors*((((r[1]-r[0]+1)**2)+8)/48))**0.5
    def probe(mcrV,probe):
        safety = exp_dist-6*exp_dist_std
        return mcrV%probe<safety
    ...
    for i in tqdm.tqdm(range(0,number_of_vectors)):
        ...
        for j in range(0,dimension_of_cla_vectors):
            if probe(mcrV[i],mcr_conv_set[j]):
                v=np.append(v,j)
